...
exports.WaitForOrderConfirmation = (driver) => {
    ...
    return driver.wait(until.urlContains('/orders/form?shop_id=968814'), 5000).then(() => {
        ...
        return driver.findElements(By.css('div.cart-items')).then((cart_items) => {
            if (cart_items.length > 0) {
                return driver.wait(until.elementLocated(By.css('fieldset.submit-set')), 1500).then((submit) => {
                    return driver.executeAsyncScript("var callback = arguments[arguments.length - 1]; arguments[0].scrollIntoView(top); callback();", submit).then(() => {
